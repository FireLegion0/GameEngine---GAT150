...
namespace nu {
	...
	public:
		ProtoCreator(std::unique_ptr<Object> prototype) :
	...
	public:
		...
		template <typename T>
			requires std::derived_from<T, Object>
		void RegisterProto(const std::string& name, std::unique_ptr<T> prototype);
	...
	inline void Factory::Register(const std::string& name) {
		...
		std::cout << "Object successfully registered: " << name << std::endl;
	...
	template<typename T>
		requires std::derived_from<T, Object>
	inline void Factory::RegisterProto(const std::string& name, std::unique_ptr<T> prototype){
		std::string lowerName = ToLower(name);
		...
		m_registry[lowerName] = std::make_unique<ProtoCreator<T>>(std::move(prototype));
	}
